...
static void dw_mci_store_cmd_log(struct mmc_command *cmd, unsigned int count, unsigned int host_idx, bool new_cmd)
{
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	if (of_alias_get_id(host->dev->of_node, "mshc") == 0) {
		...
		int sysfs_err = 0;
		host->pdata->dw_mci_cmd_logging	= true;
		host->pdata->log_count = temp_log_count;
#if 0
		// /sys/devices/15540000.dwmmc0
