...
#define USE_HMP_SEMIBOOST
#endif

#ifdef USE_HMP_SEMIBOOST
#define EGL_CPUFREQ_HMP_SEMIBOOST 1000000 /* EGL(Big) minlock frequency */
#endif
...
#ifdef USE_HMP_SEMIBOOST
static bool hmp_semiboosted = false;
#endif
					...
					const char *buf, size_t count)
{
	...
	if (cpu_input >= (int)freq_min[CA15]) {
#ifdef USE_HMP_SEMIBOOST
		/* EGL(Big) minlock frequency is lower than 1GHz, hmp semiboost is running.
									higher than 1GHz, hmp boost is running. */
		if( cpu_input <= EGL_CPUFREQ_HMP_SEMIBOOST) {
			if (!hmp_semiboosted) {
				if (set_hmp_semiboost(1) < 0)
					pr_err("%s: failed HMP Semi boost enable\n",
								__func__);
				else
					hmp_semiboosted = true;
			}
		} else {
			if (!hmp_boosted) {
		...
		/* EGL(Big) minlock frequency is lower than 1GHz, hmp semiboost is running.
									higher than 1GHz, hmp boost is running. */
		} else if(hmp_semiboosted) {
			if (set_hmp_semiboost(0) < 0)
				pr_err("%s: failed HMP Semi boost disable\n",
							__func__);
			else
				hmp_semiboosted = false;
		}
#else
		}
#endif
